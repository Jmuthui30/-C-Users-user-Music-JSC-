report 51006 "Staff Claim Voucher"
{
    ApplicationArea = All;
    Caption = 'Staff Claim Voucher';
    DefaultLayout = RDLC;
    RDLCLayout = './src/report_layout/StaffClaimVoucher.rdl';
    UsageCategory = None;
    dataset
    {
        dataitem(Payments; Payments)
        {
            ...
            begin
                ...
                PaymentMgt.FormatNoText(NumberText, Payments."Petty Cash Amount", CopyStr(Payments.Currency, 1, 10));

                Approver[1] := CopyStr(Payments."Created By", 1, 20);
                ...
                if ApprovalEntries.Find('-') then
                    repeat
                        if ApprovalEntries."Sequence No." = 1 then begin
                            Approver[2] := CopyStr(ApprovalEntries."Last Modified By User ID", 1, 20);
                            ApproverDate[2] := ApprovalEntries."Last Date-Time Modified";
                            if UserSetup1.Get(Approver[2]) then
                                UserSetup1.CalcFields(Signature);
                        end;
                        if ApprovalEntries."Sequence No." = 2 then begin
                            Approver[3] := CopyStr(ApprovalEntries."Last Modified By User ID", 1, 20);
                ...
                if Posted then begin
                    Approver[4] := CopyStr("Posted By", 1, 20);
