report 51018 "Payment Voucher-Vendor"
{
    ApplicationArea = All;
    Caption = 'Payment Voucher-Vendor';
    UsageCategory = None;
    DefaultRenderingLayout = "PV-Vendor(AACC)";
    dataset
    {
        dataitem(Payments; Payments)
        {
            ...
            column(PaymentReleaseDate_Payments; "Payment Release Date")
            {
            }
            ...
            column(Bank_Name; VendorBank.Name)
            {
            }
            ...
            column(ACKNOWLEDGEMENT_OF_RECEIPTCaptionLbl; ACKNOWLEDGEMENT_OF_RECEIPTCaptionLbl)
            {
            }
            column(ChequeCashRecievedByLbl; ChequeCashRecievedByLbl)
            {
            }
            column(IDNoPassportNoLbl; IDNoPassportNoLbl)
            {
            }
            ...
            begin
                ...
                PaymentMgt.FormatNoText(NumberText, AmountPaid, CopyStr(Currency, 1, 10));
    ...
    rendering
    {
        ...
        {
            Type = RDLC;
            LayoutFile = './src/report_layout/PaymentVoucherVendor.rdl';
            Caption = 'Payment Voucher-1';
        }
        layout(PV2)
        {
            Type = RDLC;
            LayoutFile = './src/report_layout/PV-Vendor.rdl';
            Caption = 'Payment Voucher-2';
        }
        layout("PV-Vendor(AACC)")
        {
            Type = RDLC;
            LayoutFile = './src/report_layout/PV-Vendor(AACC).rdl';
            Caption = 'Payment Voucher Vendor-AACC';
        }
    }
    ...
    var
        ...
        ChequeCashRecievedByLbl: Label 'Cheque/Cash Received By:';
        IDNoPassportNoLbl: Label 'ID No./Passport No.';
    ...
    begin
        ...
        if PLines.FindFirst() then
            if PLines."Account Type" = PLines."Account Type"::Vendor then begin
                ...
                VendAcNo := CopyStr(VendorBank."Bank Account No.", 1, MaxStrLen(VendAcNo));
                SwiftCode := VendorBank."SWIFT Code";
                BankCode := VendorBank.Code;
                BankName := VendorBank.Name;
