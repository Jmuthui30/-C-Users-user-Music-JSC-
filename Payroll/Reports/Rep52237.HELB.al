report 52237 "HELB"
{
         ...
         dataitem("Payroll PeriodX"; "Payroll Period")
        {
         ...
         column(UPPERCASE_FORMAT_DateSpecified_0___Month_Text___year4____; UpperCase(Format(DateSpecified, 0, '<Month Text> <year4>')))
            ...
            dataitem(Deductions; Deductions)
            {
                ...
                DataItemTableView = sorting(Code) where(Code = filter('D0017'));
                ...
                //     if TbInsitutions.Get(DeductionsX."Institution Code") then begin
                //         InstitutionName := TbInsitutions.Name;
                //         InstitutionAddress := TbInsitutions.Address;
                //         InstitutionCity := TbInsitutions.City;
                //         InstitutionEmail := TbInsitutions.Email;
                //     end;
                // end;
            }
   ...
   trigger OnPreReport()
    ...
    var
        ...
        DateSpecified: Date;
        TbInsitutions: record Institution;
        DedCode: Code[50];
        InstitutionNAme: Text[50];
        InstitutionAddress: Text[50];
        InstitutionEmail: Text[50];
        InstitutionTel: Text[50];
        InstitutionWebsite: Text[50];
        Institutioncity: Text[50];
        ...
        HRReportsDetails: Array[50] of Text[200];
        // CuHRMgt: Codeunit "HR Management";
        GroupSpecified: Code[30];
    ...
    local procedure GetJobPosition(Usercode: Code[50]): Text
    var
        ObjEmployee: Record Employee;
    begin
        ObjEmployee.RESET;
        ObjEmployee.SETRANGE(ObjEmployee."User ID", usercode);
        if ObjEmployee.FINDFIRST then
            EXIT(ObjEmployee."Job Position Title");
    end;
}
