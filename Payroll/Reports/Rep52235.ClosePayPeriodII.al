report 52235 "Close Pay Period II"
{
    ApplicationArea = All;
    ProcessingOnly = true;
    ...
    dataset
    {
        dataitem("Assignment Matrix-X"; "Assignment Matrix")
        {
            ...
            begin
                //   Message('start date is %1', StartingDate);
                payrollprojectallocation.Reset();
                payrollprojectallocation.SetRange(Period, StartingDate);
                payrollprojectallocation.SetRange("Employee No", "Assignment Matrix-X"."Employee No");
                if payrollprojectallocation.Find('-') then begin
                    repeat
                        // Message('payroll project allocation %1..# Close is %2', payrollprojectallocation."Employee No", payrollprojectallocation.Close);
                        payrollprojectallocation.Close := true;
                        // hapa
    ...
    var
        ...
        Newpayrollprojectallocation: Record "Payroll Project Allocation";
    ...
    begin
       ...
       by the User of the Payroll. It should ideally create new entries if the EmpRec is Active*/
        ...
        PaymentDed.SetFilter("Employee Type", '<>%1', PaymentDed."Employee Type"::Trustee);
        //PaymentDed.SETFILTER(PaymentDed.Amount, '<>%1',0);
        if PaymentDed.Find('-') then
            repeat
                CreateRec := true;
                AssignMatrix.Init();
                AssignMatrix."Employee Type" := AssignMatrix."Employee Type";
                ...
                AssignMatrix."Payroll Period" := NewPayEmployee;
                ...
                AssignMatrix.NHIF := PaymentDed.NHIF;
                AssignMatrix."Loan Product Type" := PaymentDed."Loan Product Type";
                ...
                if EmpRec.Get(PaymentDed."Employee No") then begin
                    ...
                    if (EmpRec.Status = EmpRec.Status::Active) and (EmpRec."Employee Type" <> EmpRec."Employee Type"::"Board Member") and (CreateRec = true) and (AssignMatrix.Amount <> 0)
        ...
        if PaymentDed.Find('-') then
            ...
            until PaymentDed.Next() = 0;
        //Clean any auto-created rows for the new period
        Newpayrollprojectallocation.Reset();
        Newpayrollprojectallocation.SetRange(Period, NewPeriod);
        if Newpayrollprojectallocation.FindSet() then
            Newpayrollprojectallocation.DeleteAll();

        //Carry over allocations properly
        payrollprojectallocation.Reset();
        payrollprojectallocation.SetRange(Period, PayperiodStart);
        if payrollprojectallocation.FindSet() then
            repeat
                Newpayrollprojectallocation.Init();
                // Copy everything
                Newpayrollprojectallocation.TransferFields(payrollprojectallocation, false);

                // Force all PK parts to be correct
                Newpayrollprojectallocation.Period := NewPeriod;
                Newpayrollprojectallocation."Project Code" := payrollprojectallocation."Project Code";
                Newpayrollprojectallocation."Employee No" := payrollprojectallocation."Employee No";
                Newpayrollprojectallocation."Budget Line Code" := payrollprojectallocation."Budget Line Code";
                Newpayrollprojectallocation.Allocation := payrollprojectallocation.Allocation;
                Newpayrollprojectallocation.Close := false;

                Newpayrollprojectallocation.Insert(true);
            until payrollprojectallocation.Next() = 0;
    ...
    procedure GetCurrentPeriod(var Payperiod: Record "Payroll Period II")
    ...
    begin
        ...
        StartingDate := CurrentPeriodEnd;
        PayEmployee.Reset();
        PayEmployee.SetRange("Close Pay", false);
        PayEmployee.SetFilter("Starting Date", '<>%1', StartingDate);
        if PayEmployee.FindFirst() then
            NewPayEmployee := PayEmployee."Starting Date"
        else
            ...
            repeat
                if Format(Date2DMY(NewPeriod, 2)) = Emp."Incremental Month" then
                    if IncStr(Emp."Present Pointer") < Emp.Halt then begin
                        ...
                        Emp.Validate("Present Pointer", IncStr(Emp."Present Pointer"));
                        Emp.Modify();
                    end;
            until Emp.Next() = 0;
    end;
