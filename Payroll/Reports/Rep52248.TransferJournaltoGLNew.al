report 52248 "Transfer Journal to GL-New"
{
    ApplicationArea = All;
        ...
        dataitem("Employee Posting GroupX"; "Employee HR Posting Group")
        {
            ...
            dataitem(Earning; Earning)
            {
                ...
                begin
                    ...
                    payrollprojectallocation.SetRange(payrollprojectallocation.Period, DateSpecified);
                    payrollprojectallocation.SetRange(payrollprojectallocation."Employee No", Earning."Employee Filter");
                    if payrollprojectallocation.FindSet() then
                        repeat
                            EarningsAmount := (payrollprojectallocation.Allocation / 100) * Earning."Total Amount";


                            TotalDebits := TotalDebits + "Total Amount";
                            if Earning."Total Amount" = 0 then
                                CurrReport.Skip();

                            LineNumber := LineNumber + 10000;
                            Earning.TestField(Earning."Account No.");

                            GenJnlLine.Init();
                            GenJnlLine."Journal Template Name" := BatchTemplate;
                            GenJnlLine."Journal Batch Name" := BatchName;
                            GenJnlLine."Line No." := LineNumber;
                            GenJnlLine."Account Type" := Earning."Account Type";
                            GenJnlLine."Account No." := Earning."Account No.";
                            GenJnlLine.Validate("Account No.");
                            GenJnlLine."Posting Date" := Payday;
                            GenJnlLine.Description := Earning.Description + ' ' + Format(DateSpecified, 0, '<month text> <year4>');
                            ...
                            GenJnlLine.Amount := Round(EarningsAmount);
                            GenJnlLine.Validate(Amount);
                            if GenJnlLine.Amount <> 0 then
                                GenJnlLine.Insert()
                                until payrollprojectallocation.Next() = 0;
                    TotalCredits := TotalCredits + Earning."Total Amount";
                ...
                begin
                    ...
                    Earning.SetRange(Gratuity, false);
                    Earning.SetFilter("Employee Type Filter", '<>%1', Earning."Employee Type Filter"::Trustee);

                    TotalCount := Count;
                    Counter := 0;
                end;
            }
            dataitem(Employer; Deduction)
            {
