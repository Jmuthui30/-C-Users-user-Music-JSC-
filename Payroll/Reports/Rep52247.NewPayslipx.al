report 52247 "New Payslipx"
{
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            begin
                ...
                //MESSAGE(Addr[1][2]);
                Addr[1] [3] := Employee."Employee Age";
                ...
                */
                ...
                AssignMatrix.SetRange(Type, AssignMatrix.Type::Deduction);
                AssignMatrix.SetRange(AssignMatrix."Employee No", Employee."No.");
                AssignMatrix.SetRange(AssignMatrix.NHIF, true);

                if AssignMatrix.Find('-') then begin
                    Deduct.Reset();
                    Deduct.SetRange(Deduct."Tax Deductible", true);
                    Deduct.SetRange(Deduct."Pay Period Filter", DateSpecified);
                    Deduct.SetRange(Deduct."Employee Filter", Employee."No.");
                    // Deduct.SetRange(Deduct."Show on Payslip Information", true);
                    if Deduct.FindSet() then begin
                        ArrEarnings[1, i] := 'S.H.I.F';
                        Evaluate(ArrEarningsAmt[1, i], Format(Abs(AssignMatrix."Less Pension Contribution")));
                        ArrEarningsAmt[1, i] := ChckRound(ArrEarningsAmt[1, i]);
                        // TaxableAmt := 0;
                ...
                AssignMatrix.SetRange(Type, AssignMatrix.Type::Deduction);
                AssignMatrix.SetRange(AssignMatrix."Employee No", Employee."No.");
                AssignMatrix.SetRange(AssignMatrix."Housing Levy", true);
                if AssignMatrix.Find('-') then begin
                    Assignmatrix.CALCSUMS(Amount);
                    AHLAmount := ABS(Assignmatrix.Amount);
                    //MESSAGE('AHL= %1, Deduct Code=%2',AHLAmount,DeductRecx.Code);
                    ArrEarnings[1, i] := 'Housing Levy';
              ...
              */
                ...
                // NSSFCaption := GetSHIFCaptionClass();
                i := i + 1;
                ArrEarnings[1, i] := NSSFCaption;
            ...
            trigger OnPreDataItem()
            begin
                if ExternalPeriodFilter <> '' then
                    SetFilter("Pay Period Filter", ExternalPeriodFilter);
    ...
    begin
        if ExternalPeriodFilter <> '' then
            Employee.SetFilter("Pay Period Filter", ExternalPeriodFilter);
    ...
    procedure InitPayrollFilter(var PeriodFilter: Text)
    begin
        ExternalPeriodFilter := PeriodFilter;
    end;
}