report 54455 "PRMS Report"
{
    Caption = 'PRMS Report';
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/SSRS/PRMS.rdlc';
    dataset
    {
        dataitem("Payroll Matrix"; "Assignment Matrix")
        {
            DataItemTableView = SORTING("Employee No", Type, Code, "Payroll Period", "Reference No") WHERE(Type = CONST(Deduction), Code = const('PRMS'));
            ...
            column(Assignment_Matrix_X1__Assignment_Matrix_X1__Amount; Abs("Payroll Matrix".Amount))
            {
            }
            ...
            column(Employer_Amount; Abs("Payroll Matrix"."Employer Amount"))
            { }
            ...
            column(Assignment_MatrixX1_Description; "Payroll Matrix".Description)
            {
            }
            trigger OnAfterGetRecord()
            begin
                Deductions.SetRange(Deductions."Employee No", "Payroll Matrix"."Employee No");
            ...
            begin
                DateSpecified := "Payroll Matrix".GetRangeMin("Payroll Matrix"."Payroll Period");
            end;
        }
    }
    requestpage
    ...
    trigger OnPreReport()
    begin
        // if "Payroll Matrix".GetFilter(Code) = '' then Error('Please select an earning code');
        CompInfo.Get;
