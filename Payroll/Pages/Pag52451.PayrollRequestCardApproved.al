page 52451 "Payroll Request Card-Approved"
{
    ApplicationArea = All;
    DeleteAllowed = false;
    InsertAllowed = false;
    ...
    Caption = 'Payroll Request Card-Approved';
    layout
    {
        area(content)
        {
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Disburse Advance")
            {
                ...
                trigger OnAction()
                begin
                    if Confirm('Are you sure you want to Disburse?', false) then begin
            ...
            action(Post)
            {
                ...
                trigger OnAction()
                begin
                    if Confirm('Are you sure you want to post to this month''s payroll?', false) then begin
    ...
    begin
        if (Rec.Status = Rec.Status::Approved) or (Rec.Status = Rec.Status::Rejected) or (Rec.Status = Rec.Status::Posted) then
