...
*/
                                    ...
                                    if Payments."Leave Allowance" = true then begin
                                        ...
                                        if Employee.Find('-') then begin
                                            ...
                                            if Employee."Employment Type" = Employee."Employment Type"::Contract then
                                                Amount := ((Employee."Basic Pay" * PayrollMgt.GetMonthWorked("Employee No")) * (Payments.Percentage / 100));
                                    end;
                    end;
                ...
                if Type = Type::Deduction then begin
                    if Deduction.Get(Code) then begin
                        if Deduction.Block = true then
                            Error(Error003, Deduction.Code, Deduction.Description);
                        Description := Deduction.Description;
                        "G/L Account" := Deduction."Account No.";
                        "Tax Deductible" := Deduction."Tax deductible";
                        Retirement := Deduction."Pension Scheme";
                        Shares := Deduction.Shares;
                        Paye := Deduction."PAYE Code";
                        "Secondary PAYE" := Deduction."Secondary PAYE";
                        "Insurance Code" := Deduction."Insurance Code";
                        "Main Deduction Code" := Deduction."Main Deduction Code";
                        Voluntary := Deduction.Voluntary;
                        Frequency := Deduction.Type;
                        NHIF := Deduction.NHIF;
                        "Housing Levy" := Deduction."Housing Levy";
                        //Added for Sacco Deduction
                        "Sacco Deduction" := Deduction."Sacco Deduction";

                        if Deduction.Type = Deduction.Type::Recurring then
                            "Next Period Entry" := true;
                        if Deduction."Calculation Method" = Deduction."Calculation Method"::"Flat Amount" then
                            if not Deduction.Imprest then begin
                                Amount := Deduction."Flat Amount";
                                "Employer Amount" := Deduction."Flat Amount Employer";
                            end;

                        if Deduction."Calculation Method" = Deduction."Calculation Method"::"% of Basic Pay" then
                            if Employee.Get("Employee No") then begin
                                Employee.SetRange("Pay Period Filter", "Payroll Period");
                                Employee.CalcFields("Basic Pay");
                                Amount := Deduction.Percentage / 100 * Employee."Basic Pay";
                                Amount := PayrollRounding(Amount);
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, Amount);
                                "Employer Amount" := Deduction."Percentage Employer" / 100 * Employee."Basic Pay";
                                "Employer Amount" := PayrollRounding("Employer Amount");
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, "Employer Amount");
                            end;

                        if Deduction."Calculation Method" = Deduction."Calculation Method"::"% of Gross Pay" then
                            if Employee.Get("Employee No") then begin
                                Employee.SetRange("Pay Period Filter", "Payroll Period");
                                Employee.CalcFields("Basic Pay", "Total Allowances", "Gross Excludable Allowances");
                                Amount := Deduction.Percentage / 100 * (Employee."Total Allowances" - Employee."Gross Excludable Allowances");
                                Amount := PayrollRounding(Amount);
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, Amount);
                                "Employer Amount" := Deduction."Percentage Employer" / 100 * (Employee."Total Allowances" - Employee."Gross Excludable Allowances");
                                "Employer Amount" := PayrollRounding("Employer Amount");
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, "Employer Amount");
                            end;

                        if Deduction.CoinageRounding = true then begin
                            Retirement := Deduction.CoinageRounding;
                            if Deduction."Calculation Method" = Deduction."Calculation Method"::"% of Basic Pay" then
                                "Employer Amount" := Deduction.Percentage / 100 * Employee."Basic Pay"
                            else
                                "Employer Amount" := Deduction."Flat Amount";
                            "Employer Amount" := PayrollRounding("Employer Amount");
                        end;
                    ...
                    if Deduction."Calculation Method" = Deduction."Calculation Method"::"% of Basic Pay+Hse Allowance" then
                        if Employee.Get("Employee No") then
                            if Employee.Get("Employee No") then begin
                                Employee.SetRange("Pay Period Filter", "Payroll Period");
                                Employee.CalcFields("Basic Pay", "House Allowance");
                                Amount := Deduction.Percentage / 100 * (Employee."Basic Pay" + Employee."House Allowance");
                                Amount := PayrollRounding(Amount);
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, Amount);
                                "Employer Amount" := Deduction."Percentage Employer" / 100 * (Employee."Basic Pay" + Employee."House Allowance");
                                "Employer Amount" := PayrollRounding("Employer Amount");
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, "Employer Amount");
                            end;

                    //% of Other Earnings
                    if Deduction."Calculation Method" = Deduction."Calculation Method"::"% of Other Earnings" then begin
                        TotalOtherDeductions := 0;
                        if Employee.Get("Employee No") then;
                        if Employee.Get("Employee No") then begin
                            Employee.SetRange("Pay Period Filter", "Payroll Period");
                            OtherDeductions.Reset();
                            OtherDeductions.SetRange("Main Deduction", Deduction.Code);
                            ...
                            Deduction.TestField(Percentage);
                            Amount := ((TotalOtherDeductions / 100) * Deduction.Percentage);
                            Amount := PayrollRounding(Amount);
                            "Employer Amount" := Deduction."Percentage Employer" / 100 * TotalOtherDeductions;
                            "Employer Amount" := PayrollRounding("Employer Amount");
                            CheckIfRatesInclusive("Employee No", "Payroll Period", Code, "Employer Amount");
                        end;
                    ...
                    //% of Other Deductions
                    if Deduction."Calculation Method" = Deduction."Calculation Method"::"% of Other Deductions" then begin
                        TotalDeductionLines := 0;
                        if Employee.Get("Employee No") then begin
                            Employee.SetRange("Pay Period Filter", "Payroll Period");
                            OtherDeductionLines.Reset();
                            OtherDeductionLines.SetRange("Main Code", Deduction.Code);
                            if OtherDeductionLines.FindSet() then
                                repeat
                                    AssgnMatrix.Reset();
                                    AssgnMatrix.SetRange(AssgnMatrix.Code, OtherDeductionLines."Deduction Code");
                                ...
                                until OtherDeductionLines.Next() = 0;
                            Deduction.TestField(Percentage);
                            Amount := ((TotalDeductionLines / 100) * Deduction.Percentage);
                            Amount := PayrollRounding(Amount);
                            "Employer Amount" := Deduction."Percentage Employer" / 100 * TotalDeductionLines;
                            "Employer Amount" := PayrollRounding("Employer Amount");
                            CheckIfRatesInclusive("Employee No", "Payroll Period", Code, "Employer Amount");
                        end;
                    end;
                    ...
                    if Deduction."Calculation Method" = Deduction."Calculation Method"::"% of Basic Pay+Hse Allowance + Comm Allowance + Sal Arrears" then
                        if Employee.Get("Employee No") then
                            if Employee.Get("Employee No") then begin
                                Employee.SetRange("Pay Period Filter", "Payroll Period");
                                Employee.CalcFields("Basic Pay", "House Allowance", "Commuter Allowance", "Salary Arrears");
                                Amount := Deduction.Percentage / 100 * (Employee."Basic Pay" + Employee."House Allowance" + Employee."Commuter Allowance" + Employee."Salary Arrears");
                                Amount := PayrollRounding(Amount);
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, Amount);
                                "Employer Amount" := Deduction."Percentage Employer" / 100 * (Employee."Basic Pay" + Employee."House Allowance" + Employee."Commuter Allowance" + Employee."Salary Arrears");
                                "Employer Amount" := PayrollRounding("Employer Amount");
                                CheckIfRatesInclusive("Employee No", "Payroll Period", Code, "Employer Amount");
                            end;
                    ...
                    if Deduction."Calculation Method" = Deduction."Calculation Method"::"Based on Table" then begin
                        GetPayPeriod();
                        Employee.Reset();
                        Employee.SetRange("No.", "Employee No");
                        Employee.SetRange("Pay Period Filter", "Payroll Period");
        ...
        // field(108; "Country Code"; Code[50])
        // {
    ...
    var
        ...
        DeductionsRec: Record Deduction;
        i: Integer;
    begin
        if DeductionsRec.Get(DeductionCode) then
    ...
    procedure GetBracket(DeductionsRec: Record Deduction; BasicPay: Decimal; var TierI: Decimal; var TierII: Decimal) TotalAmt: Decimal
    var
        BracketTable: Record "Brackets";
        BracketSource: Record "Bracket Tables";
        Loop: Boolean;
    ...
    begin
        ...
        if BracketTable.Find('-') then
            case BracketSource.Type of
                ...
                BracketSource.Type::Foreign:

                    repeat
                        // if BracketTable.Amount = 0 then begin
                        //     if ((BasicPay >= BracketTable."Lower Limit") and (BasicPay <= BracketTable."Upper Limit")) then
                        //         BracketAmount := (BasicPay - BracketTable."Lower Limit") * BracketTable.Percentage / 100;
                        //         TotalAmt := BracketAmount;
                        // end
                        // else
                        if ((BasicPay >= BracketTable."Lower Limit") and (BasicPay <= BracketTable."Upper Limit")) then
                            BracketAmount := (BasicPay - BracketTable."Lower Limit") * BracketTable.Percentage / 100 + BracketTable.Amount;
                        TotalAmt := BracketAmount;
                        if BracketTable.Exceeding then begin
                            BracketTable.SetFilter(Amount, '<>%1', 0);
                            if BracketTable.FindLast() then begin
                                BracketAmountII := (BasicPay - BracketTable."Lower Limit") *
                                  BracketTable.Percentage / 100 + BracketTable.Amount;
                                TotalAmt := BracketAmount + BracketAmountII;
                            end;
                        end;
    ...
    var
        Deds: Record Deduction;
        Earns: Record Earning;
    begin
        begin
            Earns.Reset();
