table 53004 "Payroll Loan Application"
{
    ...
    procedure CalculateRepayment(Rate: Decimal; Principal: Decimal; Term: Decimal; RepaymentFrequency: Decimal) Repayment: Decimal
    var
        i: Decimal;
    begin
    ...
    var
        ...
        RemainingPrincipalAmountAmortizedDec: Decimal;
    ...
    begin
        ...
        CalcInterest := LoanTypeRec."Calculate Interest";

        RoundPrecisionDec := LoanTypeRec."Rounding Precision";
        //
        //EVALUATE(GP,FORMAT("Grace Period"));
        //
        RemainingPrincipalAmountAmortizedDec := "Approved Amount";
        RunningDate := "Issued Date";

        repeat
            if CalcInterest then
                InterestAmountDec := Round(RemainingPrincipalAmountAmortizedDec / 100 / 12 * "Interest Rate", RoundPrecisionDec, RoundDirectionCode)
            else
                InterestAmountDec := 0;
            ...
            NewSchedule."Instalment No" := LineNumInt;
            ...
            if LineNumInt = Installments then begin
                NewSchedule."Remaining Debt" := 0;
                NewSchedule."Monthly Repayment" := RemainingPrincipalAmountAmortizedDec + NewSchedule."Monthly Interest";
                NewSchedule."Principal Repayment" := NewSchedule."Monthly Repayment" - NewSchedule."Monthly Interest";

                LoopEndBool := true;
            end;

            if (Repayment - InterestAmountDec) >= RemainingPrincipalAmountAmortizedDec then begin
                NewSchedule."Principal Repayment" := RemainingPrincipalAmountAmortizedDec;
                NewSchedule."Remaining Debt" := 0;
                LoopEndBool := true;
            end else begin
                NewSchedule."Principal Repayment" := Repayment - InterestAmountDec;
                RemainingPrincipalAmountAmortizedDec := RemainingPrincipalAmountAmortizedDec - (Repayment - InterestAmountDec);
                NewSchedule."Remaining Debt" := RemainingPrincipalAmountAmortizedDec;
            end;
            NewSchedule."Loan Deduction Code" := CopyStr("Deduction Code", 1, MaxStrLen(NewSchedule."Loan Deduction Code"));
            NewSchedule."Loan Interest Code" := CopyStr("Interest Deduction Code", 1, MaxStrLen(NewSchedule."Loan Interest Code"));
            NewSchedule.Insert();
    ...
    begin
        ...
        repeat
            if CalcInterest then
                InterestAmountDec := Round(RemainingPrincipalAmountDec / 100 / 12 * LoanTypeRec."Interest Rate", RoundPrecisionDec, RoundDirectionCode)
            else
                InterestAmountDec := 0;
    ...
    begin
        ...
        CalcInterest := LoanType."Calculate Interest";
        ...
        if "Interest Calculation Method" = "Interest Calculation Method"::"Flat Rate" then begin
            ...
            if LineNoInt < Installments + 1 then
                repeat
                    NewSchedule."Instalment No" := LineNoInt;
                    NewSchedule."Loan Customer Type" := "Loan Customer Type";
                    NewSchedule."Employee No" := "Employee No";
                    ...
                    if CalcInterest then
                        ...
                        NewSchedule."Monthly Interest" := 0;
                    NewSchedule."Monthly Repayment" := Repayment;
                    NewSchedule."Loan Category" := "Loan Product Type";
                    NewSchedule."Loan Amount" := "Approved Amount";
                    if not Suggested then
                    ...
                    if not Suggested then begin
                        ...
                        if LineNoInt = 1 then
                            RemainingPrincipalAmountDec := "Approved Amount" - "Suggested Repayment Amount"
                        else
