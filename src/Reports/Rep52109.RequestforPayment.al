report 52109 "Request for Payment"
{
    // version THL- ADV.FIN 1.0
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/SSRS/Request for Payment.rdlc';

    dataset
    {
        dataitem(Payments; "Request for Payment")
        {
            ...
            column(NameOfCreditor; Payments."Name of Creditor")
            {
            }
            column(AmountInWords; NumberText[1])
            {
            }
            column(Amount; Payments.Amount)
            {
            }
            ...
            begin
                ...
                ApprovalEntries.SetRange(ApprovalEntries."Table ID", 52111);
