report 53455 "Client Payslip new"
{
    // version THL- Client Payroll 1.0
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/SSRS/Client Payslip new.rdl';
    ...
    dataset
    {
        dataitem(Employee; "Client Employee Master")
        {
            ...
            column(Addr_1_3; Addr[1] [3])
            {
            }
            column(Addr_1_4; Addr[1] [4])
            {
            }
            column(DeptArr_1_1_; DeptArr[1, 1])
            {
            }
            column(StationArr_1_1; StationArr[1] [1])
            {
            }
            ...
            column(Design_CaptionLbl; Design_CaptionLbl)
            {
            }
            column(StationCaptionLbl; StationCaptionLbl)
            {
            }
            ...
            column(IDnocaptionLbl; IDnocaptionLbl)
            {
            }
            ...
            column(PeriodInstallment; PeriodInstallment)
            {
            }
            ...
            column(LDescription; LDescription)
            {
            }
            ...
            begin
                ...
                if NAVEmployee.Get(Employee."No.") then begin
                    Addr[1] [2] := NAVEmployee."First Name" + ' ' + NAVEmployee."Last Name";
                    //Addr[1][4]:=NAVEmployee.age
                    Addr[1] [3] := NAVEmployee.Title;
                    DeptArr[1] [1] := NAVEmployee."Global Dimension 1 Code";
                    StationArr[1] [1] := NAVEmployee."Employee Group";
                end;
                //Get Employee Age
                if HREmployee.Get(Employee."No.") then begin
                    Addr[1] [4] := format(HREmployee."Employee Age");
                    Addr[1] [3] := HREmployee."Job Title";
                    DeptArr[1] [1] := HREmployee."Global Dimension 1 Code";
                    StationArr[1] [1] := HREmployee."Employee Posting Group";
                end;
                // Get Basic Salary
                Earn.Reset;
                ...
                if PayrollMatrix.Find('-') then begin
                    ArrEarnings[1, i] := 'Less contribution benefit(Pension+SHIF+Housing Levy+PRMS)';
                ...
                if Earn.Find('-') then begin
                    ...
                    until Earn.Next = 0;
                    //if PayrollMatrix.Amount = 0 then CurrReport.Skip();
                end;
                // Taxable amount
                ...
                begin
                    repeat
                        ...
                        begin
                            repeat
                                TaxCharged := Abs(PayrollRounding(PAYE, Employee."Company Code")) + Abs(PayrollRounding(PayrollMatrix.Amount, Employee."Company Code"));
 ...
 */
    ...
    begin
        ...
        //MyPassword := UserSetup.Password;
        //if MyPassword = '' then
        //Error('You do not have access to run password protect report, contact administrator');
        //if EnterPassword.RunModal() = Action::OK then begin
        //if PasswordManagement.GetPassword() <> MyPassword then
    ...
    var
        ...
        StationArr: array[3, 1] of Text[60];
