report 52202 "Prepayment Schedule"
{
    DefaultLayout = RDLC;
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    RDLCLayout = './Reports/SSRS/Prepayment Schedule.rdlc';

    dataset
    {
        dataitem("Amortization Header"; "Amortization Header")
        {
            DataItemTableView = WHERE(Type=CONST(Prepayment));
            ...
            begin
                ...
                if "Amortization Header"."Starting Period" <> 0D then begin
                    if "Amortization Header"."No of Periods" = 0 then Error('No. of Periods the Prepayment %1 cannot be zero!', "Amortization Header".Description);
                ...
                if PrepLines.Find('-')then begin
                    repeat PeriodAmount[i]:=0;
                        PeriodName[i]:='';
                        PeriodName[i]:=PrepLines."Period Name";
    ...
    Periods: Integer;
    Text000: Label 'PREPAYMENT ANALYSIS AS AT %1';
    Title: Text;
}
