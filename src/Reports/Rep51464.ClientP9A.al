report 51464 "Client P9A"
{
    ...
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            column(Company_Pin; CompanyInfo."VAT Registration No.")
            {
            }
            column(V30__; '30%')
            {
            ...
            dataitem("Client Payroll Period"; "Payroll Period II")
            {
                ...
                column(TotalV30PerPension; TotalV30PerPension) { }
                ...
                column(DataItem33; V2__Date_left_if_during_the_year_CaptionLbl)
                {
                }
                ...
                begin
                    ...
                    TotalV30PerPension := Employee.Retirement;
                    ...
                    if Earn.Find('-') then begin
                        ...
                        // AssMatrix.SetRange(Company, CompCode);
                    ...
                    begin
                        ...
                        Earn.SetRange(Earn."Reduces Tax", Earn."Reduces Tax" = true);
                        if Earn.Find('-') then begin
                            InsuranceRelief := 0;
                            AssMatrix.Reset;
                            AssMatrix.SetRange(AssMatrix.Type, AssMatrix.Type::Earning);
                    ...
                    Deduction.Reset();
                    Deduction.SetCurrentKey(Deduction.NHIF);
                    Deduction.SetRange(Deduction.Code, 'SHIF');
                    if Deduction.Find('-') then begin
                        ...
                        AssMatrix.SetRange(AssMatrix."Employee No", Employee."No.");
                        AssMatrix.SetRange(AssMatrix."Payroll Period", "Starting Date");
                        AssMatrix.SetRange(Code, Deduction.Code);
                        // AssMatrix.SetRange(Company, CompCode);
                        if AssMatrix.Find('-') then
                    ...
                    //*********************************************AHLAmount
                    Deduction.Reset();
                    Deduction.SetCurrentKey(Deduction.Code);
                    Deduction.SetRange(Deduction.Code, 'ED117');
                    if Deduction.Find('-') then begin
                        AHLAmount := 0;
                        AssMatrix.Reset;
                        AssMatrix.SetRange(AssMatrix.Type, AssMatrix.Type::Deduction);
                    ...
                    Deduction.Reset();
                    Deduction.SetCurrentKey(Deduction.Code);
                    Deduction.SetRange(Deduction.Code, 'PRMS');
                    if Deduction.Find('-') then begin
                        PRMFAmount := 0;
                        AssMatrix.Reset;
                        AssMatrix.SetRange(AssMatrix.Type, AssMatrix.Type::Deduction);
            ...
            dataitem(Earnings; "Client Earnings")
            {
                ...
                column(DataItem174; MONTHLY_BENEFIT_____________RATE_DIFFERENCE_X_LOAN__12_X________Kshs________Lbl)
                {
                }
                ...
                column(DataItem216; CAR_BENEFIT____CaptiLbl)
                {
                }
            ...
            begin
                ...
                grandPAYE := 0;
                //"Total Quarters":=0;
                // CompanyInfo.Get(Employee.GetFilter("Company Code"));
    ...
    trigger OnPreReport()
    begin
        // if Employee.GetFilter("Company Code") = '' then Error('Please select a company to report for.');
    ...
    var
        ...
        MONTHLY_BENEFIT_____________RATE_DIFFERENCE_X_LOAN__12_X________Kshs________Lbl: Label 'MONTHLY BENEFIT            (RATE DIFFERENCE X LOAN)/12    =      .................................%     X        Kshs. ......................../12   = Kshs...............................';
        ...
        CAR_BENEFIT____CaptiLbl: Label 'CAR BENEFIT - The higher the amount of the monthly rate or the prescribed  rate of benefits is to be brought to charge:-';
