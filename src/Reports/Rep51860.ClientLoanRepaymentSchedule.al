report 51860 "Client Loan Repayment Schedule"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/SSRS/Client Loan Repayment Schedule.rdlc';

    dataset
    {
        dataitem("Client Loan Schedule"; "Client Loan Schedule")
        {
            DataItemTableView = SORTING("Loan No", "Employee No", "Repayment Date")ORDER(Ascending);
            RequestFilterFields = "Loan No", "Employee No";

            column(Repayment_Schedule1__Repayment_Schedule1___Employee_No_; "Client Loan Schedule"."Employee No")
            {
            }
            column(LoanApp__Employee_Name_; LoanApp."Employee Name")
            {
            }
            column(Repayment_Schedule1__Repayment_Schedule1___Loan_No_; "Client Loan Schedule"."Loan No")
            {
            }
            ...
            column(Repayment_Schedule1__Repayment_Schedule1___Instalment_No_; "Client Loan Schedule"."Instalment No")
            {
            }
            ...
            column(CummilativeCaption; CumulativeCaptionLbl)
            {
            }
            ...
            begin
                i:=i + 1;
                if LoanApp.Get("Client Loan Schedule"."Loan No", "Client Loan Schedule"."Loan Category")then begin
                    Duration:=LoanApp.Instalment;
                    IssuedDate:=LoanApp."Issued Date";
                    LoanCategory.Get("Client Loan Schedule"."Loan Category");
                    TotalPrincipalRepayment:=TotalPrincipalRepayment + "Client Loan Schedule"."Principal Repayment";
                    if i = 1 then LoanBalance:=LoanApp."Approved Amount"
                    else
                    begin
                        LoanBalance:=LoanApp."Approved Amount" - TotalPrincipalRepayment + "Client Loan Schedule"."Principal Repayment";
                    end;
                    CumInterest:=CumInterest + "Client Loan Schedule"."Monthly Interest";
                    CumMonthlyRepayment:=CumMonthlyRepayment + "Client Loan Schedule"."Monthly Repayment";
                    CumPrincipalRepayment:=CumPrincipalRepayment + "Client Loan Schedule"."Principal Repayment";
                    Rate:=LoanApp."Interest Rate";
                //Rate:=LoanApp."Interest Rate Per";
                end;
            end;
            ...
            begin
                ...
                SetRange("Client Loan Schedule"."Payroll Group", GroupCode);
            end;
        }
    }
    requestpage
    ...
    IssuedDate: Date;
    LoanCategory: Record "Client Loan Product";
    i: Integer;
    LoanBalance: Decimal;
    CumInterest: Decimal;
    ...
    j: Integer;
    GroupName: Text[70];
    loanGroup: Record "Client Loan Product";
