report 51439 "Transfer Journal to GL"
{
    // version THL- Payroll 1.0
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/SSRS/Transfer Journal to GL.rdlc';
    ...
    dataset
    {
        dataitem("Employee Groups"; "Employee Groups")
        {
            ...
            dataitem(Employee; "Employee Master")
            {
                ...
                column(Employee_Posting_GroupX1__Description; "Employee Groups".Description)
                {
                }
                column(Employee_Posting_GroupX1__Code; "Employee Groups".Code)
                {
                }
                ...
                begin
                    ...
                    ThisGross:=ThisGross + Employee."Total Allowances";
                end;
                ...
                /*
                    ...
                    GenJnline."Account No.":="Employee Groups"."Basic Salary Account";
                    GenJnline."Posting Date":=PayrollPeriod."Pay Date";
                    GenJnline.Description:='Earnings'+' '+FORMAT(DateSpecified,0,'<month text> <year4>')+'-'+"Employee Groups".Code;
            ...
            dataitem(Earnings; Earnings)
            {
                ...
                begin
                    ...
                    EarningsAccount:='';
                    GLMapping.Reset;
                    GLMapping.SetRange(Group, "Employee Groups".Code);
                    GLMapping.SetRange(Type, GLMapping.Type::Earning);
                    GLMapping.SetRange("ED Code", Earnings.Code);
                    if GLMapping.FindFirst then EarningsAccount:=GLMapping."Employer G/L Account"
                    else
                        Error('Employer GL Account for Employee Group %1 and Earning %1 must be set up first.', "Employee Groups".Code, Earnings.Code);
                    ...
                    GenJnline."Journal Batch Name":='SALARIES';
                    GenJnline."Line No.":=GenJnline."Line No." + 10000;
                    GenJnline."Account No.":=EarningsAccount;
                    GenJnline."Posting Date":=PayrollPeriod."Pay Date";
                    GenJnline.Description:=CopyStr(Earnings.Description + ' - ' + Format(DateSpecified, 0, '<month text> <year4>') + '-' + "Employee Groups".Code, 1, 50);
                    GenJnline."Document No.":=Payperiodtext;
                    GenJnline."Shortcut Dimension 1 Code":=Employee."Global Dimension 1 Code";
                    GenJnline.Validate("Shortcut Dimension 1 Code");
                    GenJnline."Shortcut Dimension 2 Code":=Employee."Global Dimension 2 Code";
            ...
            dataitem(Deductions; Deductions)
            {
                ...
                begin
                    ...
                    DeductionsEmployeeAccount:='';
                    DeductionsEmployerAccount:='';
                    GLMapping.Reset;
                    GLMapping.SetRange(Group, "Employee Groups".Code);
                    GLMapping.SetRange(Type, GLMapping.Type::Deduction);
                    GLMapping.SetRange("ED Code", Deductions.Code);
                    if GLMapping.FindFirst then begin
                        DeductionsEmployeeAccount:=GLMapping."Employee G/L Account";
                        DeductionsEmployerAccount:=GLMapping."Employer G/L Account";
                    end
                    else
                        Error('GL Account for Employee Group %1 and Deduction %1 must be set up first.', "Employee Groups".Code, Earnings.Code);
                    if((Deductions."Total Amount" <> 0) and (Deductions."Total Amount Employer" = 0))then begin
                        ...
                        GenJnline."Journal Batch Name":='SALARIES';
                        GenJnline."Line No.":=GenJnline."Line No." + 10000;
                        GenJnline."Account No.":=DeductionsEmployeeAccount;
                        GenJnline."Posting Date":=PayrollPeriod."Pay Date";
                        GenJnline.Description:=CopyStr(Deductions.Description + ' ' + Format(DateSpecified, 0, '<month text> <year4>') + '-' + "Employee Groups".Code, 1, 50);
                        GenJnline."Document No.":=Payperiodtext;
                        GenJnline."Shortcut Dimension 1 Code":=Employee."Global Dimension 1 Code";
                        GenJnline.Validate("Shortcut Dimension 1 Code");
                        GenJnline."Shortcut Dimension 2 Code":=Employee."Global Dimension 2 Code";
                    ...
                    if Deductions."Total Amount Employer" <> 0 then begin
                        ...
                        GenJnline."Journal Batch Name":='SALARIES';
                        GenJnline."Line No.":=GenJnline."Line No." + 10000;
                        GenJnline."Account No.":=DeductionsEmployerAccount;
                        GenJnline."Posting Date":=PayrollPeriod."Pay Date";
                        GenJnline.Description:=CopyStr(Deductions.Description + ' ' + Format(DateSpecified, 0, '<month text> <year4>') + '-' + "Employee Groups".Code, 1, 50);
                        GenJnline."Document No.":=Payperiodtext;
                        GenJnline."Shortcut Dimension 1 Code":=Employee."Global Dimension 1 Code";
            ...
            begin
                ...
                Totalgross:=0;
                ThisGross:=0;
                //"Employee Groups".TESTFIELD("Employee Groups"."Basic Salary Account");
                "Employee Groups".TestField("Employee Groups"."Income Tax account");
                "Employee Groups".TestField("Employee Groups"."Net Pay Account");
                //"Employee Groups".TESTFIELD("Employee Groups"."SSF Employer Account");
                PayablesAcc:="Employee Groups"."Net Pay Account";
                LineNumber:=LineNumber + 10;
            end;
            trigger OnPostDataItem()
            begin
            ...
            begin
                ...
                if AssignmentMat.Find('-')then begin
                    repeat GenJnline.Init;
                        ...
                        if LoanApp.Find('+')then if EmpRec.Get(AssignmentMat."Employee No")then GenJnline."Account No.":=EmpRec."Travels Customer Account";
                        GenJnline."Posting Date":=PayrollPeriod."Pay Date";
                        GenJnline.Description:=CopyStr(LoansRec.Description + ' ' + Format(DateSpecified, 0, '<month text> <year4>'), 1, 50);
                        ;
                        GenJnline."Document No.":=Payperiodtext;
                        GenJnline.Amount:=AssignmentMat.Amount;
                        GenJnline.Validate(GenJnline.Amount);
                        if LoanAccount.Get(EmpRec."Travels Customer Account")then begin
        ...
        dataitem(SalAdvance; Deductions)
        {
            ...
            begin
                ...
                if AssignmentMat.Find('-')then begin
                    repeat GenJnline.Init;
                        ...
                        GenJnline."Line No.":=GenJnline."Line No." + 10000;
                        GenJnline."Account Type":=GenJnline."Account Type"::Customer;
                        if EmpRec.Get(AssignmentMat."Employee No")then GenJnline."Account No.":=EmpRec."Travels Customer Account";
                        GenJnline."Posting Date":=PayrollPeriod."Pay Date";
                        GenJnline.Description:=CopyStr(SalAdvance.Description + ' ' + Format(DateSpecified, 0, '<month text> <year4>'), 1, 50);
                        ;
                        GenJnline."Document No.":=Payperiodtext;
                        GenJnline.Amount:=AssignmentMat.Amount;
                        GenJnline.Validate(GenJnline.Amount);
        ...
        dataitem(Summary; "Integer")
        {
            ...
            begin
                ...
                GenJnline."Posting Date":=PayrollPeriod."Pay Date";
                GenJnline.Description:=CopyStr('Salary payable' + ' ' + Format(DateSpecified, 0, '<month text> <year4>'), 1, 50);
                ;
                GenJnline."Document No.":=Payperiodtext;
    ...
    trigger OnPreReport()
    begin
        "Employee Groups".SetFilter("Pay Period Filter", '%1', Month);
        Payperiodtext:="Employee Groups".GetFilter("Employee Groups"."Pay Period Filter");
        Payperiodtext:=CopyStr(Payperiodtext, 4, 6);
        GetPeriodFilter:="Employee Groups".GetRangeMin("Employee Groups"."Pay Period Filter");
        DateSpecified:="Employee Groups".GetRangeMin("Employee Groups"."Pay Period Filter");
    ...
    PostingGroup: Record "Employee Groups";
    TaxAccount: Code[10];
    SalariesAcc: Code[10];
    PayablesAcc: Code[10];
    First: Code[10];
    ...
    AssignmentMat: Record "Payroll Matrix";
    ...
    GLMapping: Record "Payroll GL Mapping";
    EarningsAccount: Code[10];
    DeductionsEmployeeAccount: Code[10];
    DeductionsEmployerAccount: Code[10];
