...

{
    ...
    layout
    {
        ...
        addlast(Personal)
        {
            ...
            field("Date of Birth - Age"; Rec."Employee Age")
            {
                Caption = ' Age';
                Importance = Standard;
        ...
        addafter("Address & Contact")
        {
            group("Employment Information")
            {
                ...
                field("Job Position"; Rec."Job Position Title")
                {
        ...
        addlast(Payments)
        {
            ...
            field("NSSF No."; Rec."Social Security No.")
            {
                ...
                ApplicationArea = All;
                // CaptionClass = GetNSSFCaptionClass();
                Caption = 'NSSF';
            }
        ...
        addafter("Important Dates")
        {
            ...
            group(Anniversary)
            {
                ...
                field("Last Increment Date"; Rec."Last Date Increment")
                {
                    ApplicationArea = All;
                    ToolTip = 'Specifies the value of the Last Increment Date field.';
                }
                field("Next Increment Date"; Rec."Next Date Increment")
                {
                    ApplicationArea = All;
    ...
    actions
    {
        ...
        addlast(Processing)
        {
            ...
            group(Payslips)
            {
                ...
                //     trigger OnAction()
                //     var
                //         Online: Codeunit "Portal Integration";
                //         PayTxt: Text;
                //     begin
                //         PayTxt := Online.fnGetPayslip(Rec."No.", Today());
                //         Message(PayTxt);
                //     end;
                // }
    ...
    begin
        ...
        //Commented to allow web service user read Employee Data
        ...
        SetContractView();
        DisabilityView := false;
        // HRDates.GetDecimalFromDiffString(Rec."Date of Birth - Age");
    end;

    trigger OnModifyRecord(): Boolean
    begin
        // if not Confirm('To update the employee details, you must send the employee for approval. Do you want to proceed?', true) then
    ...
    begin
        if GuiAllowed then begin
            if Rec."Birth Date" <> 0D then begin
                Rec."Employee Age" := HRDates.DetermineAge(Rec."Birth Date", Today);
                Rec.Modify();
            end;
    ...
    var
        ...
        // CompanyCaption: Codeunit "Companies Caption";
    ...
    // local procedure GetSHIFCaptionClass(): Text
    // begin
    //     // exit(CompanyCaption.GetSHIFCaption(COMPANYNAME));

    // end;

    // local procedure GetNSSFCaptionClass(): Text
    // begin
    //     // exit(CompanyCaption.GetNSSFCaption(COMPANYNAME));
    // end;

    // local procedure GetKRACaptionClass(): Text
    // begin
    //     exit(CompanyCaption.GetKRACaption(COMPANYNAME));
    // end;
