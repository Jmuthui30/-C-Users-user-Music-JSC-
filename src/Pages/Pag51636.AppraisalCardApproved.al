page 51636 "Appraisal Card - Approved"
{
    // version THL- HRM 1.0
    Caption = 'Approved Appraisal Form';
    DeleteAllowed = false;
    Editable = false;
    InsertAllowed = false;
    ModifyAllowed = false;
    ...
    SourceTableView = WHERE(Status=CONST(Released), Closed=CONST(false), Archived=CONST(false));
    ...
    actions
    {
        area(processing)
        {
            ...
            action(Close)
            {
                ...
                trigger OnAction()
                begin
                    PerformanceMgt.CloseAppraisalForm(Rec);
                end;
            }
        }
    }
    var PerformanceMgt: Codeunit "Performance Management";
    Text000: Label 'You are about to review responses to the appraisal questions. Kindly note that you cannot abort this process once you start. Do you wish to continue?';
