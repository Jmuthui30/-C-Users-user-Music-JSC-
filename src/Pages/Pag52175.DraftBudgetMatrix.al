page 52175 "Draft Budget Matrix"
{
    // version BUDGET
    Caption = 'Draft Budget Matrix';
    ...
    begin
        ...
        if DateFilter <> '' then begin
            ...
            if Period2.GetRangeMax("Period End") < TheDimCodeBuf."Period End" then TheDimCodeBuf."Period End":=Period2.GetRangeMax("Period End");
        end;
    ...
    begin
        ...
        if TheBusUnit.Name <> '' then TheDimCodeBuf.Name:=TheBusUnit.Name
        else
    ...
    begin
        ...
        TheDimCodeBuf."Show in Bold":=TheDimVal."Dimension Value Type" <> TheDimVal."Dimension Value Type"::Standard;
    end;
    ...
    begin
        ...
        if BusUnitFilter <> '' then TheGLAccBudgetBuf.SetFilter("Business Unit Filter", BusUnitFilter);
        if GLAccFilter <> '' then TheGLAccBudgetBuf.SetFilter("G/L Account Filter", GLAccFilter);
        if DateFilter <> '' then TheGLAccBudgetBuf.SetFilter("Date Filter", DateFilter);
        if GlobalDim1Filter <> '' then TheGLAccBudgetBuf.SetFilter("Global Dimension 1 Filter", GlobalDim1Filter);
        if GlobalDim2Filter <> '' then TheGLAccBudgetBuf.SetFilter("Global Dimension 2 Filter", GlobalDim2Filter);
        if BudgetDim1Filter <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 1 Filter", BudgetDim1Filter);
        if BudgetDim2Filter <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 2 Filter", BudgetDim2Filter);
        if BudgetDim3Filter <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 3 Filter", BudgetDim3Filter);
        if BudgetDim4Filter <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 4 Filter", BudgetDim4Filter);
    end;
    ...
    begin
        ...
        case DimOption of DimOption::"G/L Account": if DimCodeBuf.Totaling <> '' then GLAccBudgetBuf.SetFilter("G/L Account Filter", DimCodeBuf.Totaling)
            else
                GLAccBudgetBuf.SetRange("G/L Account Filter", DimCodeBuf.Code);
        DimOption::Period: TheGLAccBudgetBuf.SetRange("Date Filter", DimCodeBuf."Period Start", DimCodeBuf."Period End");
        DimOption::"Business Unit": TheGLAccBudgetBuf.SetRange("Business Unit Filter", DimCodeBuf.Code);
        DimOption::"Global Dimension 1": if DimCodeBuf.Totaling <> '' then TheGLAccBudgetBuf.SetFilter("Global Dimension 1 Filter", DimCodeBuf.Totaling)
            else
                TheGLAccBudgetBuf.SetRange("Global Dimension 1 Filter", DimCodeBuf.Code);
        DimOption::"Global Dimension 2": if DimCodeBuf.Totaling <> '' then TheGLAccBudgetBuf.SetFilter("Global Dimension 2 Filter", DimCodeBuf.Totaling)
            else
                TheGLAccBudgetBuf.SetRange("Global Dimension 2 Filter", DimCodeBuf.Code);
        DimOption::"Budget Dimension 1": if DimCodeBuf.Totaling <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 1 Filter", DimCodeBuf.Totaling)
            else
                TheGLAccBudgetBuf.SetRange("Budget Dimension 1 Filter", DimCodeBuf.Code);
        DimOption::"Budget Dimension 2": if DimCodeBuf.Totaling <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 2 Filter", DimCodeBuf.Totaling)
            else
                TheGLAccBudgetBuf.SetRange("Budget Dimension 2 Filter", DimCodeBuf.Code);
        DimOption::"Budget Dimension 3": if DimCodeBuf.Totaling <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 3 Filter", DimCodeBuf.Totaling)
            else
                TheGLAccBudgetBuf.SetRange("Budget Dimension 3 Filter", DimCodeBuf.Code);
        DimOption::"Budget Dimension 4": if DimCodeBuf.Totaling <> '' then TheGLAccBudgetBuf.SetFilter("Budget Dimension 4 Filter", DimCodeBuf.Totaling)
            else
    ...
    begin
        ...
        if(GLBudgetEntry.GetFilter("Global Dimension 1 Code") <> '') or (GLBudgetEntry.GetFilter("Global Dimension 2 Code") <> '') or (GLBudgetEntry.GetFilter("Business Unit Code") <> '')then GLBudgetEntry.SetCurrentKey("Budget Name", "G/L Account No.", "Business Unit Code", "Global Dimension 1 Code")
        else
    ...
    begin
        ...
        if BusUnitFilter <> '' then GLAcc.SetFilter("Business Unit Filter", BusUnitFilter);
        if GLAccFilter <> '' then GLAcc.SetFilter("No.", GLAccFilter);
        if GlobalDim1Filter <> '' then GLAcc.SetFilter("Global Dimension 1 Filter", GlobalDim1Filter);
        if GlobalDim2Filter <> '' then GLAcc.SetFilter("Global Dimension 2 Filter", GlobalDim2Filter);
