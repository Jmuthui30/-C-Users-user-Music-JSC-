    ...
    begin
        if No = '' then begin
            if "Process Type" = "Process Type"::RFQ then begin
                ...
                if NoSeriesMgt.AreRelated(PurchSetup."Request for Quotation Nos.",xRec."No.Series") then
            "No.Series":=xRec."No.Series"
            else
            "No.Series":=PurchSetup."Request for Quotation Nos.";
            ...
            if "Process Type" = "Process Type"::RFP then begin
                ...
                if NoSeriesMgt.AreRelated(PurchSetup."Request for Proposals Nos.",xRec."No.Series") then
            "No.Series":=xRec."No.Series"
            else
            "No.Series":=PurchSetup."Request for Proposals Nos.";
            "No":=NoSeriesMgt.GetNextNo("No.Series",WorkDate());
                //NoSeriesMgt.InitSeries(SalesSetup."File Movement Numbers",xRec."File Movement Code",0D,"File Movement Code","No. Series");
            end;
            if "Process Type" = "Process Type"::Tender then begin
                ...
                if NoSeriesMgt.AreRelated(PurchSetup."Tenders Nos",xRec."No.Series") then
            "No.Series":=xRec."No.Series"
            else
            "No.Series":=PurchSetup."Tenders Nos";
            "No":=NoSeriesMgt.GetNextNo("No.Series",WorkDate());
                //NoSeriesMgt.InitSeries(SalesSetup."File Movement Numbers",xRec."File Movement Code",0D,"File Movement Code","No. Series");
            end;
        end;
