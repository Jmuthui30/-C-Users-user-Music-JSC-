table 52145 "Amortization Header"
{
        ...
        field(19; "Starting Period"; Date)
        {
            ...
            begin
                if "Starting Period" <> 0D then begin
                    ...
                    if "Period Length" = "Period Length"::Monthly then
                        "Ending Period" := CalcDate(Format(Round("No of Periods", 1)) + 'M', "Starting Period")
                    else if "Period Length" = "Period Length"::Quarterly then
                        "Ending Period" := CalcDate(Format(Round("No of Periods", 1)) + 'Q', "Starting Period")
                    else if "Period Length" = "Period Length"::"Semi-Annually" then
                        "Ending Period" := CalcDate(Format(Round("No of Periods", 1) * 6) + 'M', "Starting Period")
    ...
    begin
        if (No = '') then begin
            ...
            if NoSeriesMgt.AreRelated(PurchaseSetup."Posted Prepmt. Inv. Nos.",xRec."No. Series") then
            "No. Series":=xRec."No. Series"
            else
            "No. Series":=PurchaseSetup."Posted Prepmt. Inv. Nos.";
    ...
    begin
        ...
        PeriodicAmount := 0;
        NoOfPeriods := 0;
        if Round("No of Periods", 1) <> 0 then
            PeriodicAmount := Round("Prepayment Amount" / "No of Periods", 0.01)
        else
    ...
    begin
        ...
        if "No of Periods" <> 0 then
            PeriodicAmount := Round(Addition / "No of Periods", 0.01)
        else
            Error('No. of Periods Cannot be zero!');
        PrepLines.Reset;
        ...
        if PrepLines.Find('-') then begin
            repeat
                PrepLines.Addition := PeriodicAmount;
                PrepLines.Modify;
                ...
                if (PrepLines."Amount Distributed" <> 0) and (PrepLines."Distribution Base" <> 0) then begin
                    ...
                    if Distribution.FindSet then begin
                        repeat
                            Distribution.Addition := PeriodicAmount * (Distribution."Distribution Share" / PrepLines."Distribution Base");
                            Distribution.Modify;
                        until Distribution.Next = 0;
                    end;
                end;
    ...
    begin
        if Header.Get(DocNo) then begin //1
            ...
            if Lines2.FindFirst then
                ValueDate := CalcDate('CM', Lines2.Period)
            else
                ValueDate := Today;
            Lines.Reset;
            Lines.SetCurrentKey(No, Period);
                    ...
                    if Lines.Expensed = true then begin
                        if Confirm('You are about to post the additions for ' + Lines."Period Name" + '. Do you want to continue?', false) = true then begin
                            if Confirm('A general journal that will debit expense Account ' + Header."Expense Account" + ' and credit prepayment Account ' + Header."Prepayment Account" + ' with ' + Format(Lines.Addition) + ' will be generated. Do you want to continue?', false) = true then begin
                                ...
                                else begin //Distributed Addition
                                    Counter := 0;
                                    Distribution.Reset;
                                    Distribution.SetRange(No, Lines.No);
                                    Distribution.SetRange(Period, Lines.Period);
