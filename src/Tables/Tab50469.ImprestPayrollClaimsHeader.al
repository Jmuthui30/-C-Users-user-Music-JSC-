table 50469 "Imprest Payroll Claims Header"
{
                        ...
                        repeat
                            IF MemoLines."Amount LCY" <> 0 then begin
                                ...
                                if MemoLines.DSA <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."DSA Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                ...
                                if MemoLines."Air Ticket" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Air Ticket Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines.Conference <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Conference Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Ground Transport" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."G.Transport Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Cordination Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Cord. Allow Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Facilitator Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Facilitator Allow Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Secretariat Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Secretariat Allow Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Out of Pocket Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Out of Pocket Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Rapporteur Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Rapporteur Allow Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Driver Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Driver Allow Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Retreat Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Retreat Allow Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Expert Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Expert Allow Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines.Accomodation <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Accomodation Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Tuition Fee" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Tuition Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Mileage Allowance" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Mileage Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
                                ...
                                if MemoLines."Quarter Per Diem" <> 0 then begin
                                    ...
                                    if ProcurementDecision.Get(Rec."Imprest Memo", ImprestSetup."Qtr. Per Diem Expense Code") then
                                        if ProcurementDecision.Decision = ProcurementDecision.Decision::"Process to Payroll" then begin
                                            ImprestLines.Init();
                                            ImprestLines."No." := Rec."No.";
                                            ImprestLines."Line No" := Counter;
    ...
    begin
        if "No." = '' then begin
            ...
            if NoSeriesMgt.AreRelated(ImprestMemoSetup."Imprest Payroll Claim Nos",xRec."No. Series") then
            "No. Series":=xRec."No. Series"
            else
            "No. Series":=ImprestMemoSetup."Imprest Payroll Claim Nos";
