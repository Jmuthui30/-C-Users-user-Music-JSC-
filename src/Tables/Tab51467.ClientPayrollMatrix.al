table 51467 "Client Payroll Matrix"
{
                    ...
                    Type::Payment:
                        begin
                            if Payments.Get(Company, Code) then begin
                                ...
                                IF Payments."Calculation Method" = Payments."Calculation Method"::"% of Mortgage" then begin
                                    Assignmatrix.RESET;
                            ...
                            if Deductions.Get(Company, Code) then begin
                                ...
                                if Deductions.CoinageRounding = true then begin
                                    ...
                                    if Deductions."Calculation Method" = Deductions."Calculation Method"::"% of Basic Pay" then
                                        "Employer Amount" := Deductions.Percentage / 100 * Empl.Basic
                                    else
            ...
            begin
                ...
                if Empl.Get("Employee No") then begin
                    if Empl.Status = Empl.Status::Interdicted then begin
                        ...
                        else if Payments."House Allowance Code" = true then
                            Amount := Rec.Amount
                        else
                            Amount := Rec.Amount * 0;
                        Amount := Rec.Amount;
    ...
    var
        Empl: Record "Client Employee Master";
    ...
    procedure GetBracket(DeductionsRec: Record "Client Deductions"; BasicPay: Decimal; var TierI: Decimal; var TierII: Decimal) TotalAmt: Decimal
    var
                                    ...
                                    if Deductions."Pension Scheme" then begin
                                        if i = 1 then begin
                                            if (BracketTable.Percentage = 0) and (BracketTable.Amount <> 0) then
                                                TierI := BracketTable.Amount
                                            else
                                                TierI := TableAmount;
                                        end
                                        else begin
                                            if (BracketTable.Percentage = 0) and (BracketTable.Amount <> 0) then
                                                TierII := BracketTable.Amount
                                            else
                                                TierII := TableAmount;
                                        end;
                                    end;
