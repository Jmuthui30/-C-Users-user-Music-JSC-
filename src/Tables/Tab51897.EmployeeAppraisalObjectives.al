table 51897 "Employee Appraisal Objectives"
{
                        ...
                        if Employee.Get(UserSetup."Employee No.") then begin
                            ...
                            if EmpNav.Get("Employee No") then begin
                                ...
                                if AppraisalType.Find('-') then
                                    repeat
                                        if ((EmpNav.Scale >= AppraisalType."Minimum Job Group") and (EmpNav.Scale <= AppraisalType."Maximum Job Group")) then begin
    ...
    begin
        if "Objective No" = '' then begin
            ...
            if NoSeriesMgt.AreRelated(HumanResSetup."Appraisal Objective Nos",xRec."No. Series") then
            "No. Series":=xRec."No. Series"
            else
            "No. Series":=HumanResSetup."Appraisal Objective Nos";
            "Objective No":=NoSeriesMgt.GetNextNo("No. Series",WorkDate());
        end;
        Date := Today;
        Status := Status::Open;
        ...
        if ApraisalPeriods.Find('-') then begin
            repeat
                if (ApraisalPeriods."Start Date" <= Date) and (ApraisalPeriods."End Date" >= Date) then "Appraisal Period" := ApraisalPeriods.Period;
