table 51803 "Store Transaction Header"
{
    ...
    begin
        if "No." = '' then begin
            ...
            if Transaction = Transaction::Issue then
                // NoSeriesMgt.InitSeries(CertSetup."Store Issue Nos.", xRec."No.", 0D, "No.", "No. Series")
                if NoSeriesMgt.AreRelated(CertSetup."Store Issue Nos.", xRec."No. Series") then begin
                    "No. Series" := xRec."No. Series";
                end
                else begin
                    "No. Series" := CertSetup."Store Issue Nos.";
                    "No." := NoSeriesMgt.GetNextNo("No. Series", WorkDate());
                end
            else if Transaction = Transaction::Receive then
                // NoSeriesMgt.InitSeries(CertSetup."Store Receipt Nos.", xRec."No.", 0D, "No.", "No. Series")
                if NoSeriesMgt.AreRelated(CertSetup."Store Receipt Nos.", xRec."No. Series") then begin
                    "No. Series" := xRec."No. Series";
                end
                else begin
                    "No. Series" := CertSetup."Store Receipt Nos.";
                    "No." := NoSeriesMgt.GetNextNo("No. Series", WorkDate());
                end
                else
                 if Transaction = Transaction::Transfer then 
                //  NoSeriesMgt.InitSeries(CertSetup."Store Transfer Nos.", xRec."No.", 0D, "No.", "No. Series");
                if NoSeriesMgt.AreRelated(CertSetup."Store Transfer Nos.", xRec."No. Series") then begin
                "No. Series" := xRec."No. Series";
            end
            else begin
                "No. Series" := CertSetup."Store Transfer Nos.";
                "No." := NoSeriesMgt.GetNextNo("No. Series", WorkDate());
            end
        end;
        "Created By" := UserId;
        ...
        if UserSetup.Get(UserId) then
            if OurEmp.Get(UserSetup."Employee No.") then begin
