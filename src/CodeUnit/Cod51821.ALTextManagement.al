codeunit 51821 "AL TextManagement"
{
    // version NAVW113.00
    SingleInstance = true;
    ...
    var TodayText: Label 'TODAY', Comment = 'Must be uppercase hint reuse the transtaltion from cod1 for 2009 sp1';
    ...
    [Scope('Cloud')]
    procedure MakeDateTimeText(var DateTimeText: Text): Integer var
        Date: Date;
        Time: Time;
    begin
        if GetSeparateDateTime(DateTimeText, Date, Time)then begin
            if Date = 0D then exit(0);
            if Time = 0T then Time:=000000T;
            DateTimeText:=Format(CreateDateTime(Date, Time));
        end;
        exit(0);
    end;
    [Scope('Cloud')]
    procedure GetSeparateDateTime(DateTimeText: Text; var Date: Date; var Time: Time): Boolean var
        DateText: Text[250];
        TimeText: Text[250];
        Position: Integer;
        Length: Integer;
    begin
        if DateTimeText in[NowText, 'NOW']then DateTimeText:=Format(CurrentDateTime);
        Date:=0D;
        Time:=0T;
        Position:=1;
        ...
        if DateText = '' then exit(true);
        if MakeDateText(DateText) = 0 then;
        if not Evaluate(Date, DateText)then exit(false);
        if TimeText = '' then exit(true);
        if MakeTimeText(TimeText) = 0 then;
        if Evaluate(Time, TimeText)then exit(true);
    end;
    [Scope('Cloud')]
    procedure MakeDateText(var DateText: Text): Integer var
        Date: Date;
        PartOfText: Text;
        Position: Integer;
        Length: Integer;
    begin
        ...
        if not FindText(PartOfText, DateText, Position, Length)then exit(0);
        case PartOfText of CopyStr('TODAY', 1, StrLen(PartOfText)), CopyStr(TodayText, 1, StrLen(PartOfText)): Date:=Today;
        CopyStr('WORKDATE', 1, StrLen(PartOfText)), CopyStr(WorkdateText, 1, StrLen(PartOfText)): Date:=WorkDate;
        else
            exit(0);
        end;
    ...
    [Scope('Cloud')]
    procedure MakeTimeText(var TimeText: Text): Integer var
        PartOfText: Text[132];
        Position: Integer;
        Length: Integer;
    begin
        ...
        if not FindText(PartOfText, TimeText, Position, Length)then exit(0);
        if PartOfText <> CopyStr(TimeText, 1, StrLen(PartOfText))then exit(0);
        Position:=Position + StrLen(PartOfText);
        ReadCharacter(' ', TimeText, Position, Length);
        if Position <= Length then exit(Position);
        TimeText:=Format(000000T + Round(Time - 000000T, 1000));
        exit(0);
    end;
    [Scope('Cloud')]
    procedure MakeText(var Text: Text): Integer var
    ...
    begin
        ...
        if not ReadSymbol('?', Text, Position, Length)then exit(0);
        PartOfText:=CopyStr(Text, Position);
        if PartOfText = '' then begin
            if PAGE.RunModal(0, StandardText) = ACTION::LookupOK then Text:=StandardText.Description;
            exit(0);
        end;
        StandardText.Code:=CopyStr(Text, Position, MaxStrLen(StandardText.Code));
        if not StandardText.Find('=>') or (UpperCase(PartOfText) <> CopyStr(StandardText.Code, 1, StrLen(PartOfText)))then exit(Position);
        Text:=StandardText.Description;
        exit(0);
    end;
    procedure MakeTextFilter(var TextFilterText: Text): Integer var
        Position: Integer;
        Length: Integer;
        PartOfText: Text[250];
    ...
    begin
        ...
        if HandledByEvent then exit(Position);
        Position:=1;
        Length:=StrLen(TextFilterText);
        ReadCharacter(' ', TextFilterText, Position, Length);
        if FindText(PartOfText, TextFilterText, Position, Length)then case PartOfText of CopyStr('ME', 1, StrLen(PartOfText)), CopyStr(MeText, 1, StrLen(PartOfText)): begin
                Position:=Position + StrLen(PartOfText);
                TextFilterText:=UserId;
            end;
            CopyStr('USER', 1, StrLen(PartOfText)), CopyStr(UserText, 1, StrLen(PartOfText)): begin
                Position:=Position + StrLen(PartOfText);
                TextFilterText:=UserId;
            end;
            CopyStr('COMPANY', 1, StrLen(PartOfText)), CopyStr(CompanyText, 1, StrLen(PartOfText)): begin
                Position:=Position + StrLen(PartOfText);
                TextFilterText:=CompanyName;
            end;
            CopyStr('MYCUSTOMERS', 1, StrLen(PartOfText)), CopyStr(MyCustomersText, 1, StrLen(PartOfText)): begin
                Position:=Position + StrLen(PartOfText);
                GetMyFilterText(TextFilterText, DATABASE::"My Customer");
            end;
            CopyStr('MYITEMS', 1, StrLen(PartOfText)), CopyStr(MyItemsText, 1, StrLen(PartOfText)): begin
                Position:=Position + StrLen(PartOfText);
                GetMyFilterText(TextFilterText, DATABASE::"My Item");
            end;
            CopyStr('MYVENDORS', 1, StrLen(PartOfText)), CopyStr(MyVendorsText, 1, StrLen(PartOfText)): begin
                Position:=Position + StrLen(PartOfText);
    ...
    [Scope('Cloud')]
    procedure MakeDateTimeFilter(var DateTimeFilterText: Text): Integer var
        FilterText: Text;
    begin
    ...
    begin
        ...
        if StringPosition = 0 then begin
            if not GetSeparateDateTime(DateTimeFilterText, Date1, Time1)then exit;
            if Date1 = 0D then exit;
        ...
        if not GetSeparateDateTime(CopyStr(DateTimeFilterText, 1, StringPosition - 1), Date1, Time1)then exit;
        if not GetSeparateDateTime(CopyStr(DateTimeFilterText, StringPosition + 2), Date2, Time2)then exit;
        if(Date1 = 0D) and (Date2 = 0D)then exit;
        if Date1 <> 0D then begin
            if Time1 = 0T then Time1:=000000T;
            DateTime1:=CreateDateTime(Date1, Time1);
        end;
        if Date2 <> 0D then begin
            if Time2 = 0T then Time2:=235959T;
            DateTime2:=CreateDateTime(Date2, Time2);
        end;
        DateTimeFilterText:=Format(DateTime1) + '..' + Format(DateTime2);
    end;
    [Scope('Cloud')]
    procedure MakeDateFilter(var DateFilterText: Text): Integer begin
        MakeFilterExpression(FilterType::Date, DateFilterText);
        OnAfterMakeDateFilter(DateFilterText);
        exit(0);
    end;
    local procedure MakeDateFilterInternal(var DateFilterText: Text): Integer var
        Date1: Date;
        Date2: Date;
        Text1: Text[30];
        Text2: Text[30];
    ...
    begin
        ...
        if DateFilterText = '' then exit(0);
        StringPosition:=StrPos(DateFilterText, '..');
        if StringPosition = 0 then begin
            i:=MakeDateFilter2(OK, Date1, Date2, DateFilterText);
            if i <> 0 then exit(i);
            if OK then if Date1 = Date2 then DateFilterText:=Format(Date1)
                else
        ...
        if i <> 0 then exit(StringPosition + i - 1);
        if OK then Text2:=Format(Date2);
        DateFilterText:=Text1 + '..' + Text2;
        exit(0);
    end;
    local procedure MakeDateFilter2(var OK: Boolean; var Date1: Date; var Date2: Date; DateFilterText: Text): Integer var
        PartOfText: Text[250];
    ...
    begin
        ...
        begin
            ...
            if Position = 0 then Position:=StrPos(DateFilterText, '-');
            if Position > 0 then begin
                RemainderOfText:=DelChr(CopyStr(DateFilterText, Position));
                if Evaluate(DateFormula, RemainderOfText)then DateFilterText:=DelChr(CopyStr(DateFilterText, 1, Position - 1))
                else
                    RemainderOfText:='';
            end;
        end;
        ...
        if PartOfText <> '' then case PartOfText of CopyStr('PERIOD', 1, StrLen(PartOfText)), CopyStr(PeriodText, 1, StrLen(PartOfText)): OK:=FindPeriod(Date1, Date2, false, DateFilterText, PartOfText, Position, Length);
            CopyStr('YEAR', 1, StrLen(PartOfText)), CopyStr(YearText, 1, StrLen(PartOfText)): OK:=FindPeriod(Date1, Date2, true, DateFilterText, PartOfText, Position, Length);
            CopyStr('TODAY', 1, StrLen(PartOfText)), CopyStr(TodayText, 1, StrLen(PartOfText)): OK:=FindDate(Today, Date1, Date2, DateFilterText, PartOfText, Position, Length);
            CopyStr('WORKDATE', 1, StrLen(PartOfText)), CopyStr(WorkdateText, 1, StrLen(PartOfText)): OK:=FindDate(WorkDate, Date1, Date2, DateFilterText, PartOfText, Position, Length);
            CopyStr('NOW', 1, StrLen(PartOfText)), CopyStr(NowText, 1, StrLen(PartOfText)): OK:=FindDate(DT2Date(CurrentDateTime), Date1, Date2, DateFilterText, PartOfText, Position, Length);
            CopyStr('YESTERDAY', 1, StrLen(PartOfText)), CopyStr(YesterdayText, 1, StrLen(PartOfText)): OK:=FindDate(CalcDate('<-1D>'), Date1, Date2, DateFilterText, PartOfText, Position, Length);
            CopyStr('TOMORROW', 1, StrLen(PartOfText)), CopyStr(TomorrowText, 1, StrLen(PartOfText)): OK:=FindDate(CalcDate('<1D>'), Date1, Date2, DateFilterText, PartOfText, Position, Length);
            CopyStr('WEEK', 1, StrLen(PartOfText)), CopyStr(WeekText, 1, StrLen(PartOfText)): OK:=FindDates('<-CW>', '<CW>', Date1, Date2, DateFilterText, PartOfText, Position, Length);
            CopyStr('MONTH', 1, StrLen(PartOfText)), CopyStr(MonthText, 1, StrLen(PartOfText)): OK:=FindDates('<-CM>', '<CM>', Date1, Date2, DateFilterText, PartOfText, Position, Length);
            CopyStr('QUARTER', 1, StrLen(PartOfText)), CopyStr(QuarterText, 1, StrLen(PartOfText)): OK:=FindDates('<-CQ>', '<CQ>', Date1, Date2, DateFilterText, PartOfText, Position, Length);
            end
        else if(DateFilterText <> '') and Evaluate(Date1, DateFilterText)then begin
                Date2:=Date1;
                OK:=true;
                Position:=0;
            end
            else if RemainderOfText <> '' then begin
                    Date1:=Today;
                    Date2:=Date1;
                    OK:=true;
                    Position:=0;
    ...
    [Scope('Cloud')]
    procedure MakeTimeFilter(var TimeFilterText: Text): Integer var
        FilterText: Text;
    begin
    ...
    begin
        ...
        if StringPosition = 0 then begin
            if not GetTime(Time1, TimeFilterText)then exit;
            if Time1 = 0T then begin
        ...
        if not GetTime(Time1, CopyStr(TimeFilterText, 1, StringPosition - 1))then exit;
        if not GetTime(Time2, CopyStr(TimeFilterText, StringPosition + 2))then exit;
        if Time1 = 0T then Time1:=000000T;
        if Time2 = 0T then Time2:=235959T;
    ...
    begin
        case TypeOfFilter of FilterType::DateTime: MakeDateTimeFilter2(FilterText);
        FilterType::Date: MakeDateFilterInternal(FilterText);
        FilterType::Time: MakeTimeFilter2(FilterText);
        end;
    end;
    [Scope('Cloud')]
    procedure EvaluateIncStr(StringToIncrement: Code[20]; ErrorHint: Text)
    begin
        if IncStr(StringToIncrement) = '' then Error(UnincrementableStringError, ErrorHint);
    end;
    [Scope('Cloud')]
    procedure UnincrementableStringError(): Text begin
        exit(UnincrementableStringErr)end;
    local procedure GetTime(var Time0: Time; FilterText: Text): Boolean begin
        FilterText:=DelChr(FilterText);
    ...
    local procedure FindPeriod(var Date1: Date; var Date2: Date; FindYear: Boolean; DateFilterText: Text; PartOfText: Text; var Position: Integer; Length: Integer): Boolean var
    ...
    begin
        ...
        if AccountingPeriod.IsEmpty then begin
            if FindYear then AccountingPeriodMgt.InitStartYearAccountingPeriod(AccountingPeriod, WorkDate)
            else
            ...
            ReadCharacter(' ', DateFilterText, Position, Length);
            if Position > Length then Position:=0;
            exit(true);
        end;
        if FindYear then AccountingPeriod.SetRange("New Fiscal Year", true)
        else
            AccountingPeriod.SetRange("New Fiscal Year");
        Sign:='';
        if ReadSymbol('+', DateFilterText, Position, Length)then Sign:='+'
        else if ReadSymbol('-', DateFilterText, Position, Length)then Sign:='-';
        if Sign = '' then if ReadNumeral(Numeral, DateFilterText, Position, Length)then begin
                if FindYear then AccountingPeriod.FindFirst
                else
                begin
        ...
        begin
            if not ReadNumeral(Numeral, DateFilterText, Position, Length)then exit(true);
            if Sign = '-' then Numeral:=-Numeral;
            AccountingPeriod."Starting Date":=WorkDate;
            AccountingPeriod.Find('=<');
            AccountingPeriod.Next(Numeral);
        end;
        Date1:=AccountingPeriod."Starting Date";
        if AccountingPeriod.Next = 0 then Date2:=DMY2Date(31, 12, 9999)
        else
    ...
    local procedure FindDate(Date1Input: Date; var Date1: Date; var Date2: Date; DateFilterText: Text; PartOfText: Text; var Position: Integer; Length: Integer): Boolean begin
        Position:=Position + StrLen(PartOfText);
        ReadCharacter(' ', DateFilterText, Position, Length);
        Date1:=Date1Input;
        Date2:=Date1;
    ...
    local procedure FindDates(DateFormulaText1: Text; DateFormulaText2: Text; var Date1: Date; var Date2: Date; DateFilterText: Text; PartOfText: Text; var Position: Integer; Length: Integer): Boolean var
    ...
    local procedure FindText(var PartOfText: Text; Text: Text; Position: Integer; Length: Integer): Boolean var
        Position2: Integer;
    begin
        Position2:=Position;
        ReadCharacter(AlphabetText, Text, Position, Length);
        if Position = Position2 then exit(false);
        PartOfText:=UpperCase(CopyStr(Text, Position2, Position - Position2));
        exit(true);
    end;
    local procedure ReadSymbol(Token: Text[30]; Text: Text; var Position: Integer; Length: Integer): Boolean begin
        if Token <> CopyStr(Text, Position, StrLen(Token))then exit(false);
        Position:=Position + StrLen(Token);
        ReadCharacter(' ', Text, Position, Length);
        exit(true);
    end;
    local procedure ReadNumeral(var Numeral: Integer; Text: Text; var Position: Integer; Length: Integer): Boolean var
        Position2: Integer;
        i: Integer;
    begin
        Position2:=Position;
        ReadCharacter('0123456789', Text, Position, Length);
        if Position2 = Position then exit(false);
        Numeral:=0;
        for i:=Position2 to Position - 1 do if Numeral < 1000 then Numeral:=Numeral * 10 + StrPos('0123456789', CopyStr(Text, i, 1)) - 1;
        if(Numeral < 1) or (Numeral > 999)then Error(NumeralOutOfRangeError);
        exit(true);
    end;
    local procedure ReadCharacter(Character: Text[50]; Text: Text; var Position: Integer; Length: Integer)
    begin
        while(Position <= Length) and (StrPos(Character, UpperCase(CopyStr(Text, Position, 1))) <> 0)do Position:=Position + 1;
    end;
    ...
    var
        ...
        NoOfValues: Integer;
    begin
        if not(MyTableNo in[DATABASE::"My Customer", DATABASE::"My Vendor", DATABASE::"My Item"])then exit;
        TextFilterText:='';
        NoOfValues:=0;
        RecRef.Open(MyTableNo);
        FieldRef:=RecRef.Field(1);
        FieldRef.SetRange(UserId);
        if RecRef.Find('-')then repeat FieldRef:=RecRef.Field(2);
                AddToFilter(TextFilterText, Format(FieldRef.Value));
                NoOfValues+=1;
            until(RecRef.Next = 0) or (NoOfValues > 2000);
        RecRef.Close;
        if NoOfValues > 2000 then Message(OverflowMsg);
    end;
    local procedure AddToFilter(var FilterString: Text; MyNo: Code[20])
    begin
        if FilterString = '' then FilterString:=MyNo
        else
            FilterString+='|' + MyNo;
    end;
    [Scope('Cloud')]
    procedure RemoveMessageTrailingDots(Message: Text): Text begin
        exit(DelChr(Message, '>', '.'));
    end;
    [Scope('Cloud')]
    procedure GetRecordErrorMessage(ErrorMessageField1: Text[250]; ErrorMessageField2: Text[250]; ErrorMessageField3: Text[250]; ErrorMessageField4: Text[250]): Text begin
    ...
    begin
        ...
        if StrLen(ErrorText) > 250 then ErrorMessageField2:=CopyStr(ErrorText, 251, 250);
        if StrLen(ErrorText) > 500 then ErrorMessageField3:=CopyStr(ErrorText, 501, 250);
        if StrLen(ErrorText) > 750 then ErrorMessageField4:=CopyStr(ErrorText, 751, 250);
    end;
    procedure XMLTextIndent(InputXMLText: Text): Text var
    ...
    [Scope('Cloud')]
    procedure Replace(InputText: Text; ToReplace: Text; ReplacementText: Text): Text // var
