codeunit 50006 "Commitment Management"
{
    ...
    procedure GetAvailableBudget(BudgetLine: Code[20]; RefDate: Date; var DimensionOne: Code[20]; var DimensionTwo: Code[20]): Decimal
    var
        BudgetAmount: Decimal;
        Expenses: Decimal;
        BudgetAvailable: Decimal;
    ...
    begin
        ...
        if InsufficientExists then
            Memo."Budget Available" := false
        else
    ...
    local procedure GetExpenseCodeAccount(var ExpenseCode: Code[20]): Code[20]
    var
        ExpenseCodes: Record "Expense Codes";
    begin
        if ExpenseCodes.Get(ExpenseCode) then
            exit(ExpenseCodes."Account No")
        else
            exit('');
    end;
    ...
    begin
        ...
        if not Memo.Committed then begin
            ...
            if MemoLines.FindFirst then begin
                if Committments.FindLast then EntryNo := Committments."Entry No";
                repeat
                    if MemoLines."Amount Required" <> 0 then begin
