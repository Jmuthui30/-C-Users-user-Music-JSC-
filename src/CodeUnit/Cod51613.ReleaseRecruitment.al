codeunit 51613 "Release Recruitment"
{
    // version THL- HRM 1.0
    ...
    procedure Reopen(var Recruitment: Record "Recruitment Request")
    begin
        if Recruitment.Status = Recruitment.Status::Open then exit;
        ClearReleaseFields(Recruitment);
        Recruitment.Status:=Recruitment.Status::Open;
        Recruitment.Modify(true);
    end;
    procedure Reject(var Recruitment: Record "Recruitment Request")
    begin
        ClearReleaseFields(Recruitment);
        Recruitment.Status:=Recruitment.Status::Open;
        Recruitment.Rejected:=true;
        Recruitment.Modify(true);
    end;
    procedure PerformManualRelease(var Recruitment: Record "Recruitment Request")
    var
        ApprovalsMgmt: Codeunit "Approvals Mgmt.";
    begin
        CODEUNIT.Run(CODEUNIT::"Release Recruitment", Recruitment);
    end;
    procedure PerformManualReopen(var Recruitment: Record "Recruitment Request")
    begin
        if Recruitment.Status = Recruitment.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reopen(Recruitment);
    end;
    procedure PerformManualReject(var Recruitment: Record "Recruitment Request")
    begin
        if Recruitment.Status = Recruitment.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reject(Recruitment);
    end;
    local procedure ClearReleaseFields(var Recruitment: Record "Recruitment Request")
    begin
    ...
    procedure OnAfterReleaseRecruitment(var Recruitment: Record "Recruitment Request")
    begin
    end;
}
