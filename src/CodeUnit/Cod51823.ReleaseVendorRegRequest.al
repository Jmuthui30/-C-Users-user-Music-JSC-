codeunit 51823 "Release Vendor Reg. Request"
{
    ...
    procedure Reopen(var VendorReg: Record "Vendor Reg. Request")
    begin
        if VendorReg.Status = VendorReg.Status::Open then exit;
        ClearReleaseFields(VendorReg);
        VendorReg.Status:=VendorReg.Status::Open;
        VendorReg.Modify(true);
    end;
    procedure Reject(var VendorReg: Record "Vendor Reg. Request")
    begin
        ClearReleaseFields(VendorReg);
        VendorReg.Status:=VendorReg.Status::Open;
        VendorReg.Modify(true);
    end;
    procedure PerformManualRelease(var VendorReg: Record "Vendor Reg. Request")
    var
        ApprovalsMgmt: Codeunit "Approvals Mgmt.";
    begin
        CODEUNIT.Run(CODEUNIT::"Release Vendor Reg. Request", VendorReg);
    end;
    procedure PerformManualReopen(var VendorReg: Record "Vendor Reg. Request")
    begin
        if VendorReg.Status = VendorReg.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reopen(VendorReg);
    end;
    procedure PerformManualReject(var VendorReg: Record "Vendor Reg. Request")
    begin
        if VendorReg.Status = VendorReg.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reject(VendorReg);
    end;
    local procedure ClearReleaseFields(var VendorReg: Record "Vendor Reg. Request")
    begin
    ...
    procedure OnAfterReleaseVendorReg(var VendorReg: Record "Vendor Reg. Request")
    begin
    end;
}
