codeunit 52109 "Release Supplementary Budget"
{
    ...
    // Sacco:   61623 - 62422 = ***
    // ***********************
    TableNo = "Supplementary Budget Request";
    ...
    begin
        ...
        Rec.Modify(true);
        OnAfterReleaseSupplementaryBudget(Rec);
    end;
    ...
    procedure Reopen(var SupplementaryBudget: Record "Supplementary Budget Request")
    begin
        if SupplementaryBudget.Status = SupplementaryBudget.Status::Open then exit;
        ClearReleaseFields(SupplementaryBudget);
        SupplementaryBudget.Status:=SupplementaryBudget.Status::Open;
        SupplementaryBudget.Modify(true);
    end;
    procedure Reject(var SupplementaryBudget: Record "Supplementary Budget Request")
    begin
        ClearReleaseFields(SupplementaryBudget);
        SupplementaryBudget.Status:=SupplementaryBudget.Status::Open;
        SupplementaryBudget.Modify(true);
    end;
    procedure PerformManualRelease(var SupplementaryBudget: Record "Supplementary Budget Request")
    var
        ApprovalsMgmt: Codeunit "Approvals Mgmt.";
    begin
        CODEUNIT.Run(CODEUNIT::"Release Supplementary Budget", SupplementaryBudget);
    end;
    procedure PerformManualReopen(var SupplementaryBudget: Record "Supplementary Budget Request")
    begin
        if SupplementaryBudget.Status = SupplementaryBudget.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reopen(SupplementaryBudget);
    end;
    procedure PerformManualReject(var SupplementaryBudget: Record "Supplementary Budget Request")
    begin
        if SupplementaryBudget.Status = SupplementaryBudget.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reject(SupplementaryBudget);
    end;
    local procedure ClearReleaseFields(var SupplementaryBudget: Record "Supplementary Budget Request")
    begin
    ...
    procedure OnAfterReleaseSupplementaryBudget(var SupplementaryBudget: Record "Supplementary Budget Request")
    begin
    end;
}
