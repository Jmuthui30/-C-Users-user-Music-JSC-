codeunit 51606 "Release Orientation Checklist"
{
    // version THL- HRM 1.0
    ...
    // Sacco:   61623 - 62422 = ***
    // *************************
    TableNo = "Staff Orientation Header";
    ...
    procedure Reopen(var Orientation: Record "Staff Orientation Header")
    begin
        if Orientation.Status = Orientation.Status::Open then exit;
        ClearReleaseFields(Orientation);
        Orientation.Status:=Orientation.Status::Open;
        Orientation.Modify(true);
    end;
    procedure Reject(var Orientation: Record "Staff Orientation Header")
    begin
        ClearReleaseFields(Orientation);
        Orientation.Status:=Orientation.Status::Open;
        Orientation.Modify(true);
    end;
    procedure PerformManualRelease(var Orientation: Record "Staff Orientation Header")
    var
        ApprovalsMgmt: Codeunit "Approvals Mgmt.";
    begin
        CODEUNIT.Run(CODEUNIT::"Release Orientation Checklist", Orientation);
    end;
    procedure PerformManualReopen(var Orientation: Record "Staff Orientation Header")
    begin
        if Orientation.Status = Orientation.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reopen(Orientation);
    end;
    procedure PerformManualReject(var Orientation: Record "Staff Orientation Header")
    begin
        if Orientation.Status = Orientation.Status::"Pending Approval" then Error(CancelOrCompleteToReopenDocErr);
        Reject(Orientation);
    end;
    local procedure ClearReleaseFields(var Orientation: Record "Staff Orientation Header")
    begin
    ...
    procedure OnAfterReleaseOrientation(var Orientation: Record "Staff Orientation Header")
    begin
    end;
}
